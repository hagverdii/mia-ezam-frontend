...
const OtherOperationsPage = () => {
	...
	useEffect(() => {
		if (!getAllPosResultsQuery.isLoading) {
			setPosResultOptions(
				getAllPosResultsQuery.data.data.map((posRes) => {
					return {
	...
	useEffect(() => {
		if (!getAllConclusionsQuery.isLoading) {
			setConclusionOptions(
				getAllConclusionsQuery.data.data.map((conclusion) => {
					return {
	...
	return (
		<>
			<dialog
				className='other-operations-dialog'
				ref={dialogRef}
			>
						...
						<div>
							...
							<div style={{ display: 'flex', gap: '.5rem' }}>
								...
								<button
									disabled={
										!selectedPurpose ||
										deletePurposeMutation.isPending ||
										addInProgress
									}
									onClick={() => {
						...
						<div>
							...
							<div style={{ display: 'flex', gap: '.5rem' }}>
								...
								<button
									disabled={
										!selectedPosResult ||
										deletePosResultMutation.isPending ||
										addInProgress
									}
									onClick={() => {
										deletePosResultMutation.mutate({
											id: selectedPosResult.value,
										})
									}}
									className='delete-button'
								>
						...
						<div>
							...
							<div style={{ display: 'flex', gap: '.5rem' }}>
								...
								<button
									disabled={
										!selectedConclusion ||
										deleteConclusionMutation.isPending ||
										addInProgress
									}
									onClick={() => {
										deleteConclusionMutation.mutate({
											id: selectedConclusion.value,
										})
									}}
									className='delete-button'
								>
