...
const BusinessTripsPageableTable = () => {
    ...
    useEffect(() => {
        if (fullName) {
            const cleanSearch = fullName.trim().replace(/\s+/g, ' ').replace(/[^a-zA-Z0-9İiəƏüÜşŞçÇğĞöÖıI\s]+/g, '')
            setSearchParams(prev => {
    ...
    const handleSearch = (event) => {
        ...
        setSearchParams(prev => {
            prev.set('fullName', searchBar.trim().replace(/\s+/g, ' ').replace(/[^a-zA-Z0-9İiəƏüÜşŞçÇğĞöÖıI\s]+/g, ''))
            prev.set('pageNumber', '0')
            return prev
        })
    }
            ...
            <div className='businessTrips-search-container'>
                ...
                <div>
                    <select value={searchDay} onChange={e => setSearchDay(e.target.value)}>
                        ...
                        {Array.from({ length: ( searchMonth==='' ||
                                                ...
                                                searchMonth==='12') ? 31 :
                                                    searchMonth==='02' &&
                                                    searchYear!=='' &&
                                                    (searchYear % 100 === 0 ? searchYear % 400 === 0 : searchYear % 4 === 0)
                                                        ? 29
                                                        : 28}).map((_, index) => {
                                                            return <option key={nanoid()} value={`${index+1 < 10 ? 0 : ''}${index+1}`}>{index+1}</option>
                        })}
                    </select>
                    ...
                    <select value={searchYear} onChange={e => setSearchYear(e.target.value)}>
                        <option value=''></option>
                        {Array.from({ length: (new Date()).getFullYear() - 2015}).map((_, index) => {
                            const value = (new Date()).getFullYear()-index
                            return <option key={nanoid()} value={value}>{value}</option>
                        })}
                    </select>
            ...
            <div className='businessTrips-table'>
                        ...
                        : <table>
                            ...
                            <tbody>
                                {getBusinessTripsQuery.data?.data?.content.length
                                    ? getBusinessTripsQuery.data?.data?.content.map((trip, index) => {
                                        return (
                                            <tr key={nanoid()}>
                                                <td>{Number(pageNumber)*Number(pageSize)+index+1}</td>
                                                <td>{trip.employeeMoneyDetails.map(employeeDetail => {
                                                    return <p className='employee-details' key={nanoid()}><strong>{employeeDetail?.employee?.lastName + ' '
                                                            + employeeDetail?.employee?.firstName + ' '
                                                            + employeeDetail?.employee?.fatherName}</strong>{' - ['
                                                        + employeeDetail?.employee?.rank?.name + ', '
                                                        + employeeDetail?.employee?.department?.name + ']'}</p>
                                                })}</td>
                                                <td><p>{trip.startingDate}</p></td>
                                                <td>{trip.businessTripDetails.map(tripDetail => {
                                                    return <p key={nanoid()}>{tripDetail.region.name + ' - ' +
                                                    tripDetail.dayCount + ' gün'}</p>
                                                })}</td>
                ...
                <div className='bottom-row'>
                    <div className='page-navigation'>
                        <button className='default-button' disabled={(!getBusinessTripsQuery.isLoading ? getBusinessTripsQuery.data?.data?.first : true)} onMouseDown={e => setSearchParams(prev => {
                            prev.set('pageNumber', 0)
                            return prev
                        })}>
                            <DoubleBackIcon />
                        ...
                        <div>
                            ...
                            <select
                                ...
                                value={pageNumber}
                            >
                                {!getBusinessTripsQuery.isLoading && Array.from({ length: getBusinessTripsQuery.data?.data?.totalPages }, (_, index) => (
                        ...
                        <button className='default-button' disabled={(!getBusinessTripsQuery.isLoading ? getBusinessTripsQuery.data?.data?.last : true)} onClick={e => setSearchParams(prev => {
                            prev.set('pageNumber', `${Number(pageNumber)+1}`)
                            return prev
                        })}>
                            <ForwardIcon />
                        ...
                        <button className='default-button' disabled={(!getBusinessTripsQuery.isLoading ? getBusinessTripsQuery.data?.data?.last : true)} onClick={e => setSearchParams(prev => {
                            prev.set('pageNumber', `${getBusinessTripsQuery.data?.data?.totalPages - 1}`)
                            return prev
                        })}>
                            <DoubleForwardIcon />
                        </button>
