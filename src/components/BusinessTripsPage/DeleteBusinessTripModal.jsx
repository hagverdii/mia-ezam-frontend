...
const DeleteBusinessTripModal = ({
	...
	const deleteTripByIdMutation = useMutation({
		mutationFn: ({ deleteTripId }) =>
			deleteBusinessTripById(auth.jwtToken, deleteTripId),
		onSuccess: (data) => {
			setSelectedTrip({})
			notifySuccess()
		},
	...
	const handleSubmit = (e) => {
		...
		try {
			if (
				getBusinessTripsQuery.data?.data?.content?.length === 1 &&
				pageNumber > 0
			) {
	...
	return (
		...
		>
			<div className='dialog-container'>
				...
				<div className='title'>
					<strong>
						Ezamiyyəti siyahıdan silmək üçün əməliyyatı təsdiq edin
					</strong>
				</div>
				<form onSubmit={handleSubmit}>
					<button
						disabled={deleteTripByIdMutation.isPending}
