			...
			<div className='employees-table'>
					...
					<div>
						<form onSubmit={handleSearch}>
							<input
								...
								onFocus={() =>
									setPlaceholderText(
										'Soyad, ad, ata adı, rütbə, şöbə, vəzifə daxil edin'
									)
								}
				...
				<div className='bottom-row'>
					<div className='page-navigation'>
						<button
							className='default-button'
							disabled={!isLoading ? data?.data?.first : true}
							onMouseDown={(e) =>
								setSearchParams((prev) => {
									prev.set('pageNumber', 0)
									return prev
						...
						<div>
							...
							>
								{!isLoading &&
									Array.from({ length: data.data.totalPages }, (_, index) => (
										<option
											key={index + 1}
											value={index}
										>
						...
						<button
							className='default-button'
							disabled={!isLoading ? data?.data?.last : true}
							onClick={(e) =>
								setSearchParams((prev) => {
									prev.set('pageNumber', `${Number(pageNumber) + 1}`)
									return prev
